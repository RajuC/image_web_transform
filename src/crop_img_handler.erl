...
handle(Req, State) ->
	...
	io:format("~ncrop_img_handler::PostVals : ~p~n",[PostVals]),	
	[{<<"crop">>,Trans}] = PostVals,
	[CropTrans,Image,Extn] = string:tokens(binary_to_list(Trans),"_"),	
	ImageName = Image++"."++Extn,
	ImageRec = image_transform:identify_file(ImageName),
    Url = "http://localhost:8088/images/"++Image++"_orig."++Extn,
	HtmlBody = trans_html:provide_html(ImageRec,CropTrans,Url),
